# AWS S3 Upload Python example

This folder contains a Python application example that handles S3 buckets on AWS (Amazon Web Services).

Upload a local file to a S3 bucket using the Client API (low-level) of Boto 3.
...
* Run the code.
  ...
  * `<OBJECT_NAME>`     by Object file name in the bucket.
  * `<LOCAL_FILE_NAME>` by local file name.

  Run application:

  ```bash
  python s3upload.py <BUCKET_NAME> <OBJECT_NAME> <LOCAL_FILE_NAME>
  ```
