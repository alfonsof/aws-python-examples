...
def run_instance():
  ...
  ec2resource = boto3.resource('ec2', region_name=region)
  # Run an instance
  instances = ec2resource.create_instances(ImageId=ami_id, InstanceType=instance_type,
...
def start_instance(instance_id):
  ...
  try:
    # Start an instance
    response = ec2resource.Instance(instance_id).start(DryRun=False)
    print(response)
...
def stop_instance(instance_id):
  ...
  try:
    # Stop an instance
    response = ec2resource.Instance(instance_id).stop(DryRun=False)
    print(response)
...
def reboot_instance(instance_id):
  ...
  try:
    # Reboot an instance
    response = ec2resource.Instance(instance_id).reboot(DryRun=False)
    print(response)
...
def terminate_instance(instance_id):
  ...
  try:
    ...
    response = ec2resource.Instance(instance_id).terminate(DryRun=False)
    print(response)
