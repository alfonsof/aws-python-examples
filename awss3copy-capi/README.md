...
Copy an object from a S3 bucket to another S3 bucket using the Client API (low-level) of Boto 3.
...
* Run the code.
  ...
  * `<DESTINATION_BUCKET>` by destination bucket name.
  ...
  ```bash
  python s3copy.py <SOURCE_BUCKET> <SOURCE_FILE> <DESTINATION_BUCKET>
  ```
